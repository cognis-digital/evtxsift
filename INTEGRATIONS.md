...
**evtxsift** plugs into your stack through [`cognis-connect`](https://github.com/cognis-digital/cognis-connect),
...
Once `evtxsift` emits JSON findings, pipe them straight to a destination — `--dry-run`
previews the exact request without sending:

```bash
evtxsift ... --format json | cognis-connect emit --to stix   # STIX 2.1 bundle
evtxsift ... --format json | cognis-connect emit --to misp --url $URL --token $TOK   # MISP event
evtxsift ... --format json | cognis-connect emit --to splunk --url $URL --token $TOK   # Splunk HEC
evtxsift ... --format json | cognis-connect emit --to elastic --url $URL --token $TOK   # Elastic _bulk
```
...
works whatever `evtxsift` outputs:

```python
from cognis_connect import normalize, stix
findings = [normalize(rec, source="evtxsift") for rec in records]   # records = your JSON output
