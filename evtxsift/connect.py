"""Native cognis-connect emit for evtxsift — forward findings to any platform.

Maps evtxsift's JSON output to the canonical `Finding` and forwards it via
...
Usage:
    evtxsift ... --format json | evtxsift-emit --to stix
    evtxsift-emit --to slack --url $WEBHOOK --dry-run < findings.json
"""
...
def map_record(rec: dict) -> dict:
    ...
    try:
        out = dict(rec)
        out['title'] = rec.get('event_id', '') + ' - ' + rec.get('task_name', '')
        out['severity'] = 'info'
        if 'brute-force' in rec.get('event_data', ''):
            out['severity'] = 'high'
        elif 'persistence' in rec.get('event_data', ''):
            out['severity'] = 'medium'
        elif 'lateral-movement' in rec.get('event_data', ''):
            out['severity'] = 'high'
        out['type'] = 'event'
        out['description'] = rec.get('event_data', '')
        out['tags'] = []
        if rec.get('event_id'):
            out['tags'].append('event-id-' + rec['event_id'])
        if rec.get('task_name'):
            out['tags'].append('task-name-' + rec['task_name'])
        if rec.get('ipv4'):
            out['tags'].append('ipv4-' + rec['ipv4'])
        if rec.get('domain'):
            out['tags'].append('domain-' + rec['domain'])
        if rec.get('url'):
            out['tags'].append('url-' + rec['url'])
        if rec.get('sha256'):
            out['tags'].append('sha256-' + rec['sha256'])
        if rec.get('cve'):
            out['tags'].append('cve-' + rec['cve'])
        if rec.get('imo'):
            out['tags'].append('imo-' + rec['imo'])
        if rec.get('mmsi'):
            out['tags'].append('mmsi-' + rec['mmsi'])
        if rec.get('lat'):
            out['tags'].append('lat-' + rec['lat'])
        if rec.get('lon'):
            out['tags'].append('lon-' + rec['lon'])
        return out
    except Exception:
        return rec
